...
public class CustomInput
{
    ...
    public float touchDeltaDist;

    public void update_double_touch_method(Touch touch1, Touch touch2)
    {
        float previousTouchDist = 0;
        if (touchDist != 0)
        {
            previousTouchDist = touchDist;
        }
        touchDist = (touch1.position - touch2.position).magnitude;
        if (previousTouchDist != 0)
        {
            touchDeltaDist = touchDist - previousTouchDist;
        }
    }
...
public class UI2DController : MonoBehaviour {
    ...
    void touch_and_activate(MouseClickData curTouch)
    {
        ...
        else if (curTouch.phase == MouseClickData.MouseState.NO_PRESS)
        {
            itemCurTouching = null;
        }
    }
	...
	void Update () {
        ...
        if (myType == TouchControllerType.TOUCH_ACTIVATE)
        {
            ...
            else if (Input.GetKey(KeyCode.Mouse0) && Input.GetKeyUp(KeyCode.Mouse0) || pcInputActive)
