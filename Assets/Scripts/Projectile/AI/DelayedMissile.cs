...
public class DelayedMissile : MyProjectile
{
    ...
    void Start()
    {
        ...
        float delayOffset = ((float)Random.Range(1, 40) / 100.0f);
        Debug.Log("Random Poll" + Random.Range(1, 20));
        Debug.Log("Missile delay offset: " + delayOffset);
        delayTime = Time.time + delayOffset;
        track = true;
    }
