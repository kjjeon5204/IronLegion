...
public class NinjaShotgun : Character {
	...
	public override void manual_update () {
		...
		if ((currentStates == CurrentStates.IDLE && globalCDTracker < Time.time)) {
			...
			if (randomNum >= 0 && randomNum < 60) {
				currentStates = CurrentStates.ARMORBREAK;
				abilityList[0].initialize_ability();
			}
			else if (randomNum >= 40 && randomNum < 100) {
				currentStates = CurrentStates.CHARGESHOT;
		...
		//Event Handler
		if (currentStates != CurrentStates.MOVE && currentStates != CurrentStates.ARMORBREAK) {
			custom_look_at();
		}
		...
		else if (currentStates == CurrentStates.ARMORBREAK) {
		...
		else if (currentStates == CurrentStates.CHARGESHOT) {
