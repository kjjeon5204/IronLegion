...
public class UIDialogueBaseSystem : MonoBehaviour {
    ...
    public virtual bool initialize_dialogue_system()
    {
    ...
    bool check_word_fit(string tempWord)
    {
        ...
        Vector3 positionShifter = fullTextDisplay[currentLineDisplay].renderer.bounds.extents;
        positionShifter.z = 0;
        textBuffer.gameObject.transform.position = fullTextDisplay[currentLineDisplay].transform.position
            + positionShifter;
    ...
    {
        if (dialogueProgress < dialogueData.Length)
        {
